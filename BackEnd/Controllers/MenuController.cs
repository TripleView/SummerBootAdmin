...
public class MenuController : ControllerBase
{
    ...
    public async Task<ApiResult<MenuOutPutDto>> GetMenus()
    {
        ...
        if (!HttpContext.User.IsInRole("admin"))
        {
            ...
            {
                var userMenus = await userRoleRepository.InnerJoin(roleRepository, x => x.T1.RoleId == x.T2.Id)
                    .InnerJoin(roleAssignMenuRepository, x => x.T2.Id == x.T3.RoleId)
                    .InnerJoin(menuRepository, x => x.T4.Id == x.T3.MenuId)
