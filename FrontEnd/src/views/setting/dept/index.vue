...
export default {
	...
	data() {
		return {
			...
			apiObj: this.$API.department.tree,
			selection: [],
			search: {
				keyword: null
			}
	...
	methods: {
		...
		async table_del(row) {
			...
			//  {id: row.id}
			var res = await this.$API.department.deleteDepartments.post(reqData);
			if (res.code == 20000) {
		...
		async batch_del() {
			...
			this.$confirm(`确定删除选中的 ${this.selection.length} 项吗？如果删除项中含有子集将会被一并删除`, '提示', {
				...
				var res = await that.$API.department.deleteDepartments.post(reqData);
